...
// Map Unity Physics objects into System.Object, so the methods can be used in standalone mode
using Collision = BehaviorDesigner.Runtime.Standalone.ICollision;
using Collision2D = BehaviorDesigner.Runtime.Standalone.ICollision;
using Collider = BehaviorDesigner.Runtime.Standalone.ICollision;
using Collider2D = BehaviorDesigner.Runtime.Standalone.ICollision;
using ControllerColliderHit = BehaviorDesigner.Runtime.Standalone.ICollision;
#else
using UnityEngine;
#endif
...
#else
    public abstract class Behavior : BehaviorDesigner.Runtime.Standalone.BehaviorGameObject, IBehavior
#endif
    {
        [SerializeField]
        ...
        public ExternalBehavior ExternalBehavior
        {
            ...
            set
            {
                behaviourManager.DisableBehavior(this);
        ...
        [SerializeField]
        public BehaviorSource mBehaviorSource;
        ...
        #if BEHAVIOR_DESIGNER_STANDALONE
        public System.Object GetObject() { return this; }
        #else
        ...
        public enum EventTypes { OnCollisionEnter, OnCollisionExit, OnTriggerEnter, OnTriggerExit,
                                 ...
                                 OnControllerColliderHit, OnLateUpdate, OnFixedUpdate, None }
...
#if BEHAVIOR_DESIGNER_STANDALONE
        BehaviorManager behaviourManager = null;
        BinaryDeserialization binaryDeserialization = null;

#else
        BehaviorManager behaviourManager
        {
            get
            {
...
#if BEHAVIOR_DESIGNER_STANDALONE
        public Behavior(BehaviorManager abehaviourManager, BinaryDeserialization abinaryDeserialization)
        {
            behaviourManager = abehaviourManager;
            binaryDeserialization = abinaryDeserialization;
            mBehaviorSource = new BehaviorSource(this);
            mBehaviorSource.binaryDeserialization = binaryDeserialization;
        }
#else
        public Behavior()
        {
        ...
        private bool TaskContainsMethod(string methodName, Task task)
        {
            ...
            var method = type.GetMethod(methodName, System.BindingFlags.Public |System.BindingFlags.NonPublic | System.BindingFlags.Instance | System.BindingFlags.DeclaredOnly);
#else
            var method = type.GetMethod(methodName, System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.DeclaredOnly);
#endif
            if (method != null && method.DeclaringType.IsAssignableFrom(type)) {
                return true;
            }
        ...
        {
            behaviourManager.EnableBehavior(this);
        ...
        {
            behaviourManager.DisableBehavior(this, pauseWhenDisabled);
            isPaused = pauseWhenDisabled;
        }

        public void DisableBehavior(bool pause)
        {
            behaviourManager.DisableBehavior(this, pause);
            isPaused = pause;
        }
        ...
        {
            behaviourManager.DestroyBehavior(this);
        }
        ...
        {
            if (hasEvent[(int)EventTypes.OnCollisionEnter]) {
                behaviourManager.BehaviorOnCollisionEnter(collision, this);
            }
        }

        public void OnCollisionExit(Collision collision)
        {
            if (hasEvent[(int)EventTypes.OnCollisionExit]) {
                behaviourManager.BehaviorOnCollisionExit(collision, this);
            }
        }

        public void OnTriggerEnter(Collider other)
        {
            if (hasEvent[(int)EventTypes.OnTriggerEnter]) {
                behaviourManager.BehaviorOnTriggerEnter(other, this);
            }
        }

        public void OnTriggerExit(Collider other)
        {
            if (hasEvent[(int)EventTypes.OnTriggerExit]) {
                behaviourManager.BehaviorOnTriggerExit(other, this);
            }
        }

        public void OnCollisionEnter2D(Collision2D collision)
        {
            if (hasEvent[(int)EventTypes.OnCollisionEnter2D]) {
                behaviourManager.BehaviorOnCollisionEnter2D(collision, this);
            }
        }

        public void OnCollisionExit2D(Collision2D collision)
        {
            if (hasEvent[(int)EventTypes.OnCollisionExit2D]) {
                behaviourManager.BehaviorOnCollisionExit2D(collision, this);
            }
        }

        public void OnTriggerEnter2D(Collider2D other)
        {
            if (hasEvent[(int)EventTypes.OnTriggerEnter2D]) {
                behaviourManager.BehaviorOnTriggerEnter2D(other, this);
            }
        }

        public void OnTriggerExit2D(Collider2D other)
        {
            if (hasEvent[(int)EventTypes.OnTriggerExit2D]) {
                behaviourManager.BehaviorOnTriggerExit2D(other, this);
            }
        }

        public void OnControllerColliderHit(ControllerColliderHit hit)
        {
            if (hasEvent[(int)EventTypes.OnControllerColliderHit]) {
                behaviourManager.BehaviorOnControllerColliderHit(hit, this);
            }
        }

#if !BEHAVIOR_DESIGNER_STANDALONE && (UNITY_EDITOR || DLL_DEBUG || DLL_RELEASE)
        ...
        public List<Task> GetActiveTasks()
        {
            ...
            return behaviourManager.GetActiveTasks(this);
        }

        // System.objects don't normally support coroutines. Add that support here.
        public object StartTaskCoroutine(Task task, string methodName)
        {
#if !UNITY_EDITOR && (UNITY_WSA_8_0 || UNITY_WSA_8_1)
        ...
        private void RegisterEvent(string name, Delegate handler)
        {
            ...
            if (!eventTable.TryGetValue(type, out eventHandlers)) {
                eventHandlers = new Dictionary<string, Delegate>();
                eventTable.Add(type, eventHandlers);
            }
        ...
        {
            if (task == null || index >= defaultValues.Count) {
                return;
            }

            var taskValues = defaultValues[index];
            index++;
        ...
        public static BehaviorManager CreateBehaviorManager()
        {
            ...
            {
                var newBehaviorManager = new BehaviorManager();
                newBehaviorManager.Init(new ObjectPool());
                newBehaviorManager.Print += (data) => { Console.WriteLine(data); };
                newBehaviorManager.name = "Behavior Manager";
                return newBehaviorManager;
            }
#else
            if (BehaviorManager.instance == null && UnityEngine.Application.isPlaying)
            {
                var behaviorManager = new UnityEngine.GameObject();
                //behaviorManager.hideFlags = HideFlags.HideAndDontSave;
                behaviorManager.name = "Behavior Manager";
                BehaviorManager.instance = behaviorManager.AddComponent<BehaviorManager>();
                BehaviorManager.instance.Init(new ObjectPool());
                return BehaviorManager.instance;
            }
#endif
            return null;
