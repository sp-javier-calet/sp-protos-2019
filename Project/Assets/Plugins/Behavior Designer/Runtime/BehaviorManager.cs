...
namespace BehaviorDesigner.Runtime
{
    ...
    {
#if !BEHAVIOR_DESIGNER_STANDALONE
        static public BehaviorManager instance = null;
#endif
        ...
        public class BehaviorTree
        {
            ...
            public void Initialize(ObjectPool objectPool, Behavior b)
            {
                behavior = b;
        ...
        public void Init(ObjectPool aobjectPool)
        {
        ...
        private MethodInfo playMakerStopMethod = null;
        private MethodInfo PlayMakerStopMethod
        {
        ...
        private MethodInfo uScriptStopMethod = null;
        private MethodInfo UScriptStopMethod
        {
        ...
        private MethodInfo dialogueSystemStopMethod = null;
        private MethodInfo DialogueSystemStopMethod
        {
        ...
        private MethodInfo uSequencerStopMethod = null;
        private MethodInfo USequencerStopMethod
        {
        ...
        private MethodInfo iCodeStopMethod = null;
        private MethodInfo ICodeStopMethod
        {
        ...
        private object[] invokeParameters = new object[1];
        ...
        public void EnableBehavior(Behavior behavior)
        {
            ...
            taskAddData.Initialize(objectPool);
            ...
            behaviorTree.Initialize(objectPool, behavior);
            ...
            if (behaviorTree.behavior.LogTaskChanges) {
                for (int i = 0; i < behaviorTree.taskList.Count; ++i) {
                    Print(string.Format("{0}: Task {1} ({2}, index {3}) {4}", RoundedTime(), behaviorTree.taskList[i].FriendlyName, behaviorTree.taskList[i].GetType(), i, behaviorTree.taskList[i].GetHashCode()));
                }
            }
#endif
        ...
        private int AddToTaskList(BehaviorTree behaviorTree, Task task, ref bool hasExternalBehavior, TaskAddData data)
        {
            ...
            task.Behavior = behaviorTree.behavior;
            task.BehaviorManager = this;
#if !BEHAVIOR_DESIGNER_STANDALONE
...
#if !UNITY_EDITOR && NETFX_CORE
                if (fields[i].FieldType.GetTypeInfo().IsClass && !fields[i].FieldType.Equals(typeof(Type)) && !typeof(Delegate).IsAssignableFrom(fields[i].FieldType)) {
#else
                if (fields[i].FieldType.IsClass && !fields[i].FieldType.Equals(typeof(Type)) && !typeof(Delegate).IsAssignableFrom(fields[i].FieldType)) {
#endif
        ...
        private SharedVariable OverrideSharedVariable(BehaviorTree behaviorTree, TaskAddData data, Type fieldType, SharedVariable sharedVariable)
        {
            ...
            if (!string.IsNullOrEmpty(sharedVariable.Name) && data.overrideFields.TryGetValue(sharedVariable.Name, out overrideField)) {
                ...
                if (overrideField.Value is SharedVariable) {
                    ...
                    if (overrideSharedVariable.IsShared) {
        ...
        public void DisableBehavior(Behavior behavior, bool paused)
        {
            ...
            if (behavior.LogTaskChanges) {
                Print(string.Format("{0}: {1} {2}", RoundedTime(), (paused ? "Pausing" : "Disabling"), behavior.ToString()));
            }
#endif
        ...
        private void ReevaluateParentTasks(BehaviorTree behaviorTree)
        {
            for (int i = behaviorTree.parentReevaluate.Count - 1; i > -1; --i) {
                ...
                if (behaviorTree.taskList[parentReevaluateIndex] is Decorator) {
                    ...
                    var parentReevaluateTask = behaviorTree.taskList[parentReevaluateIndex] as ParentTask;
        ...
        private TaskStatus RunParentTask(BehaviorTree behaviorTree, int taskIndex, ref int stackIndex, TaskStatus status)
        {
            ...
            if (!parentTask.CanRunParallelChildren() || parentTask.OverrideStatus(TaskStatus.Running) != TaskStatus.Running) {
                ...
                while (parentTask.CanExecute() && (childStatus != TaskStatus.Running || parentTask.CanRunParallelChildren()) && IsBehaviorEnabled(behaviorTree.behavior)) {
                    ...
                    if ((executionsPerTick == ExecutionsPerTickType.NoDuplicates && childIndex == prevChildIndex) ||
                        ...
                        if (executionsPerTick == ExecutionsPerTickType.Count) {
                            Print(string.Format("{0}: {1}: More than the specified number of task executions per tick ({2}) have executed, returning early.", 
                ...
                if (stackIndex == 0) {
                    ...
                    if (notifyOnEmptyStack) {
                        if (behaviorTree.behavior.RestartWhenComplete) {
                            ...
                            // The behaviorTree object may be reused immeidately after it is pooled so keep a reference to the behavior component.
...
#if UNITY_EDITOR || DLL_DEBUG || DLL_RELEASE
            if (behaviorTree.behavior.LogTaskChanges) {
                Print(string.Format("{0}: Restarting {1}", RoundedTime(), behaviorTree.behavior.ToString()));
            }
#endif
            // Remove all of the conditional aborts that haven't had a chance to run yet
            RemoveChildConditionalReevaluate(behaviorTree, -1);
...
#if UNITY_EDITOR || DLL_DEBUG || DLL_RELEASE
                                if (behavior.LogTaskChanges) {
                                    Print(string.Format("{0}: {1}: Interrupt task {2} ({3}) with index {4} at stack index {5}", RoundedTime(), behaviorTree.behavior.ToString(),
        ...
        public bool MapObjectToTask(object objectKey, Task task, ThirdPartyObjectType objectType)
        {
            if (objectTaskMap.ContainsKey(objectKey)) {
                ...
                Print(string.Format("Only one behavior can be mapped to the same instance of the {0}.", thirdPartyName));
                return false;
            }
