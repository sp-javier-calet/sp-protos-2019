...
namespace BehaviorDesigner.Runtime
{
    ...
    #endif
    {
        ...
        public static Task DeserializeTask(BehaviorSource behaviorSource, Dictionary<string, object> dict, ref Dictionary<int, Task> IDtoTask, List<Object> unityObjects)
        {
            ...
            // What happened?
            if (task == null) {
                Console.WriteLine("Error: task is null of type " + dict["ObjectType"]);
                return null;
            }
            ...
            if (dict.TryGetValue("Instant", out dictObj)) {
                task.IsInstant = Convert.ToBoolean(dictObj);
            }
            if (dict.TryGetValue("Disabled", out dictObj)) {
                task.Disabled = Convert.ToBoolean(dictObj);
            }
        ...
        private static NodeData DeserializeNodeData(Dictionary<string, object> dict, Task task)
        {
            ...
            if (dict.TryGetValue("IsBreakpoint", out dictObj)) {
                nodeData.IsBreakpoint = Convert.ToBoolean(dictObj);
            }
            if (dict.TryGetValue("Collapsed", out dictObj)) {
                nodeData.Collapsed = Convert.ToBoolean(dictObj);
            }
            if (dict.TryGetValue("ColorIndex", out dictObj)) {
                nodeData.ColorIndex = Convert.ToInt32(dictObj);
            }
        ...
        private static SharedVariable DeserializeSharedVariable(Dictionary<string, object> dict, IVariableSource variableSource, bool fromSource, List<Object> unityObjects)
        {
            ...
            if (!fromSource && variableSource != null && dict.TryGetValue("Name", out nameObj)) {
                object globalObj;
                dict.TryGetValue("IsGlobal", out globalObj);
                if (!dict.TryGetValue("IsGlobal", out globalObj) || Convert.ToBoolean(globalObj) == false) {
            ...
            if (sharedVariable == null || !(typesEqual = sharedVariable.GetType().Equals(variableType))) {
                ...
                if (dict.TryGetValue("IsShared", out dictValue)) {
                    sharedVariable.IsShared = Convert.ToBoolean(dictValue);
                }
                if (dict.TryGetValue("IsGlobal", out dictValue)) {
                    sharedVariable.IsGlobal = Convert.ToBoolean(dictValue);
                }
                if (dict.TryGetValue("NetworkSync", out dictValue)) {
                    sharedVariable.NetworkSync = Convert.ToBoolean(dictValue);
                }
                if (!sharedVariable.IsGlobal && dict.TryGetValue("PropertyMapping", out dictValue)) {
                    ...
                    if (dict.TryGetValue("PropertyMappingOwner", out dictValue)) {
                        sharedVariable.PropertyMappingOwner = IndexToUnityObject(Convert.ToInt32(dictValue), unityObjects) as GameObject;
                    }
        ...
        private static void DeserializeObject(Task task, object obj, Dictionary<string, object> dict, IVariableSource variableSource, List<Object> unityObjects)
        {
            ...
            for (int i = 0; i < fields.Length; ++i) {
                object dictObject;
                if (dict.TryGetValue(fields[i].FieldType + "," + fields[i].Name, out dictObject) || dict.TryGetValue(fields[i].Name, out dictObject)) {
                    if (typeof(IList).IsAssignableFrom(fields[i].FieldType)) {
                        ...
                        if (type.Equals(typeof(Task)) || type.IsSubclassOf(typeof(Task))) {
                            if (TaskUtility.HasAttribute(fields[i], typeof(InspectTaskAttribute))) {
                                ...
                                idList.Add(Convert.ToInt32(dictObject));
                                taskIDs.Add(new TaskField(task, fields[i]), idList);
                            }
                        } else {
                            fields[i].SetValue(obj, ValueToObject(task, type, dictObject, variableSource, unityObjects));
                        }
                    }
#if (UNITY_METRO || UNITY_WINRT) && !UNITY_EDITOR
...
#endif
                    if (dict.TryGetValue(fields[i].FieldType + "," + fields[i].Name, out dictObject)) {
...
#if BEHAVIOR_DESIGNER_STANDALONE
            else if (type.Equals(typeof(MonoBehaviour)) || type.Equals(typeof(ScriptableObject)) || type.IsSubclassOf(typeof(MonoBehaviour)) || type.IsSubclassOf(typeof(ScriptableObject))) 
#else
        ...
        private static AnimationCurve ValueToAnimationCurve(Dictionary<string, object> value)
        {
            ...
            if (value.TryGetValue("Keys", out obj)) {
                ...
                for (int i = 0; i < keys.Count; ++i) {
                    ...
                    var keyframe = new Keyframe((float)Convert.ChangeType(keyValue[0], typeof(float)), (float)Convert.ChangeType(keyValue[1], typeof(float)),
                                                (float)Convert.ChangeType(keyValue[2], typeof(float)), (float)Convert.ChangeType(keyValue[3], typeof(float)));
                    keyframe.tangentMode = (int)Convert.ChangeType(keyValue[4], typeof(int));
