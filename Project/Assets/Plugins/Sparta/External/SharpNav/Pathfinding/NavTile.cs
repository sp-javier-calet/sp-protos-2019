...
namespace SharpNav.Pathfinding
{
    ...
    public class NavTile : IEquatable<NavTile>
    {
        ...
        {
            return QueryPolygons(qbounds, new NavQueryFilter(), polys);
        }
        ...
        public int QueryPolygons(BBox3 qbounds, NavQueryFilter filter, List<NavPolyId> polys)
        {
            if(BVTree.Count != 0)
            {
                int node = 0;
                ...
                while(node < end)
                {
                    ...
                    if(isLeafNode && overlap)
                    {
                        ...
                        if(filter.PassFilter(polyRef, this, this.Polys[bvNode.Index]))
                        {
                            polys.Add(polyRef);
                        }
                    }
            ...
            else
            {
                ...
                for(int i = 0; i < PolyCount; i++)
                {
                    ...
                    //Must pass filter
                    NavPolyId pRef;
                    idManager.SetPolyIndex(ref baseRef, i, out pRef);
                    if(!filter.PassFilter(pRef, this, poly))
                        continue;
            ...
            for(int j = 0; j < DetailMeshes[indexPoly].TriangleCount; j++)
            {
                ...
                for(int k = 0; k < 3; k++)
                {
                    if(t[k] < poly.VertCount)
                        _closeHeightVectorList[k] = Verts[poly.Verts[t[k]]];
                    else
                        _closeHeightVectorList[k] = DetailVerts[pd.VertexIndex + (t[k] - poly.VertCount)];
                }

                if(Distance.PointToTriangle(pos, _closeHeightVectorList[0], _closeHeightVectorList[1], _closeHeightVectorList[2], out h))
                    return true;
            }
