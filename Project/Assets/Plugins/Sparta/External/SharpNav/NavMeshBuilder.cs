...
namespace SharpNav
{
    ...
    {
        //[SP-Change] Added support for flags in polygons
        //              Warning: check code for off-mesh connections in cosntructor, it seems the author of the C# port was unsure about the settings for flags, tags and areas.
        ...
        public NavMeshBuilder(PolyMesh polyMesh, PolyMeshDetail polyMeshDetail, OffMeshConnection[] offMeshCons, NavMeshGenerationSettings settings)
        {
            ...
            for(int i = 0; i < offMeshCons.Length; i++)
            {
                ...
                if(offMeshSides[i * 2 + 0] == BoundarySide.Internal)
                {
                    ...
                    navPolys[offMeshPolyBase + n].Flags = (ushort)offMeshCons[i].Flags;
