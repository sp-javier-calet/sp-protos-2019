...
namespace SharpNav.Crowds
{
    ...
    public class LocalBoundary
    {
        ...
        public void Update(NavPolyId reference, Vector3 pos, float collisionQueryRange, NavMeshQuery navquery, NavQueryFilter filter)
        {
            ...
            navquery.FindLocalNeighborhood(ref centerPoint, collisionQueryRange, filter, polys, tempArray, ref numPolys, MaxLocalPolys);
            ...
            for(int j = 0; j < numPolys; j++)
            {
                ...
                navquery.GetPolyWallSegments(polys[j], filter, segs, tempArray, ref numSegs, MAX_SEGS_PER_POLY);
        ...
        public bool IsValid(NavMeshQuery navquery, NavQueryFilter filter)
        {
            if(numPolys == 0)
                return false;
