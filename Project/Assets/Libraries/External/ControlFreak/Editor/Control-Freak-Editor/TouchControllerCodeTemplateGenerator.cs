...
public class TouchControllerCodeTemplateGenerator  : EditorWindow
	...
	public void InitAndShow(TouchController joy)
		...
		var titleStr = "Control Freak Code Generator";
#if UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6
		this.title = titleStr;
#else
		this.titleContent = new GUIContent(titleStr);
#endif
	...
	private string AdaptString(string s)
		...
		s = s.Replace("CTRL_VAR_", 	ctrlVarStr);
...
*/
	...
	public const string 
		...
		JS_LAYOUT_CODE_BEGIN	= 
			"\t\tif (CTRL_VAR.LayoutChanged())\n" + 
			"\t\t\t{\t\n" + 
			"\t\t\tvar guiScreenRect\t: Rect\t= CTRL_VAR.GetScreenEmuRect(false);\n" + 
			"\t\t\tvar camViewportRect\t: Rect\t= CTRL_VAR.GetScreenEmuRect(true);\n" + 
			"\t\t\t\t\n" + 
			"\n",
		...
		CS_LAYOUT_CODE_BEGIN	= 
			"\t\tif (CTRL_VAR.LayoutChanged())\n" + 
			"\t\t\t{\t\n" + 
			"\t\t\tRect guiScreenRect \t\t= CTRL_VAR.GetScreenEmuRect(false);\n" + 
			"\t\t\tRect camViewportRect\t= CTRL_VAR.GetScreenEmuRect(true);\n" + 
			"\t\t\t\t\n" + 
			"\n",
		...
		CS_ZONE_MULTI_PRESSED_BEGIN_CODE =
			...
			"\t\t\tVector2\tZ_PREFIX_MultiRawDrawDelta\t= ZONE_VAR.GetMultiDragDelta(true);\n" + 
			"\n" + 
			"\n",
		...
		CS_ZONE_UNI_PRESSED_BEGIN_CODE =
			...
			"\t\t\t{\n" + 
			"\t\t\tfloat\tZ_PREFIX_UniDur\t\t\t\t= ZONE_VAR.GetMultiTouchDuration();\n" + 
			"\t\t\tVector2\tZ_PREFIX_UniPos\t\t\t\t= ZONE_VAR.GetMultiPos();\n" + 
			"\t\t\tVector2\tZ_PREFIX_UniDragDelta\t\t= ZONE_VAR.GetMultiDragDelta();\n" + 
			"\t\t\tVector2\tZ_PREFIX_UniRawDrawDelta\t= ZONE_VAR.GetMultiDragDelta(true);\n" + 
			"\n" + 
			"\n",
		...
		JS_ZONE_MULTI_PRESSED_BEGIN_CODE =
			...
			"\t\t\tvar Z_PREFIX_MultiRawDrawDelta\t: Vector2\t= ZONE_VAR.GetMultiDragDelta(true);\n" + 
			"\n" + 
			"\n",
		...
		JS_ZONE_UNI_PRESSED_BEGIN_CODE =
			...
			"\t\tif (ZONE_VAR.UniPressed())\n" + 
			"\t\t\t{\n" + 
			"\t\t\tvar Z_PREFIX_UniDur\t\t\t: float\t\t= ZONE_VAR.GetMultiTouchDuration();\n" + 
			"\t\t\tvar Z_PREFIX_UniPos\t\t\t: Vector2\t= ZONE_VAR.GetMultiPos();\n" + 
			"\t\t\tvar Z_PREFIX_UniDragDelta\t: Vector2\t= ZONE_VAR.GetMultiDragDelta();\n" + 
			"\t\t\tvar Z_PREFIX_UniRawDrawDelta\t: Vector2\t= ZONE_VAR.GetMultiDragDelta(true);\n" + 
			"\n" + 
			"\n",
