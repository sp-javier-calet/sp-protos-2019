...
namespace SocialPoint.GUIAnimation
{
	...
	public static class StepsManager 
	{
		...
		{
			new StepData(){StepType = typeof(AnchorsEffect), ConfigurableInEditor = true},
			new StepData(){StepType = typeof(PositionEffect), ConfigurableInEditor = true},
			...
			new StepData(){StepType = typeof(UniformEffect), ConfigurableInEditor = true},

			// Deprecated
			new StepData(){StepType = typeof(TransformEffect), ConfigurableInEditor = true},
		};
		public static StepData GetBlendStepData(System.Type type)
		{
			return BlendStepsData.Find( (StepData adata) => { return adata.StepType == type; } );
		}

		public static List<StepMonitorData> BlendMonitorsData = new List<StepMonitorData>()
		{
			new StepMonitorData(){StepType = typeof(TransformEffect), StepMonitorType = typeof(TransformEffect.TargetValueMonitor) },

			new StepMonitorData(){StepType = typeof(AnchorsEffect), StepMonitorType = typeof(AnchorsEffect.TargetValueMonitor) },
			new StepMonitorData(){StepType = typeof(PositionEffect), StepMonitorType = typeof(PositionEffect.TargetValueMonitor) },
			new StepMonitorData(){StepType = typeof(ScaleEffect), StepMonitorType = typeof(ScaleEffect.TargetValueMonitor) },
			new StepMonitorData(){StepType = typeof(RotationEffect), StepMonitorType = typeof(RotationEffect.TargetValueMonitor) },

			new StepMonitorData(){StepType = typeof(ColorEffect), StepMonitorType =  typeof(ColorEffect.TargetValueMonitor) },
			new StepMonitorData(){StepType = typeof(OpacityEffect), StepMonitorType =  typeof(OpacityEffect.TargetValueMonitor) },
		};
		public static StepMonitorData GeMonitorData(System.Type monitorType)
		{
			return BlendMonitorsData.Find( (StepMonitorData adata) => { return adata.StepMonitorType == monitorType; } );
		}
		...
		{
			return TriggerStepsData.Find( (StepData adata) => { return adata.StepType == type; } );
		}
		...
		{
			if(steps.Exists( (StepData astep) => { return astep.StepType == stepType; } ))
			{
				return;
			}
		...
		{
			if(StepsManager.BlendMonitorsData.Exists( (StepMonitorData astep) => { return astep.StepType == stepType; } ))
			{
				return;
			}
			StepsManager.BlendMonitorsData.Add(new StepMonitorData(){ StepType = stepType, StepMonitorType = stepMonitorType });
		}
	}
}
