...
namespace SocialPoint.GUIAnimation
{
	...
	public class Group : Step
	{
		...
		public void RemoveAnimItem<T>(T item) where T:Step
		{
			...
			bool exist = AnimItems.Find((Step i) =>{ return i == item; });
			if(exist)
			{
				item.OnRemoved();
		...
		{
			bool existInSource = source.AnimItems.Find((Step i) =>{ return i == sourceItem; });
			if(!existInSource)
			{
		...
		public int GetFirstFreeSlot(int min, int max)
		{
			...
			for (; slot < max; ++slot) 
			{
				isFree = !AnimItems.Exists( (Step animItem)=>{ return animItem.Slot == slot; } );
				if(isFree)
				{
					return slot;
				}
