...
//			Debug.Log(string.Format("Pos: {0}, anchorMin{1}, anchorMax{2}, width: {3}, height: {4}", pos, anchorMin, anchorMax, size.x, size.y ));
		...
		public static Vector2[] SetAnchorsNative(Transform trans, Vector2 relativeMin, Vector2 relativeMax, Vector2 absoluteMin, Vector2 absoluteMax)
		{
			...
			return new Vector2[] { rectTrans.anchorMin, rectTrans.anchorMax, rectTrans.offsetMin, rectTrans.offsetMax };
		}
	...
	//----
		...
		static Transform GetAnchorParentNGUI(Transform trans)
		{
			...
			if(graphic != null)
			{
				if(graphic.leftAnchor != null) return graphic.leftAnchor.target;
				if(graphic.rightAnchor != null) return graphic.rightAnchor.target;
				if(graphic.topAnchor != null) return graphic.topAnchor.target;
				if(graphic.bottomAnchor != null) return graphic.bottomAnchor.target;
			}
			return null;
		}
		...
		static Vector2[] SetAnchorsNGUI(Transform trans, Transform transParent, AnchorMode anchorMode, bool isResursive)
		{
			...
			if(hasRect)
			{
				...
				{
					// Setup the Min X
					if(Mathf.Abs(anchorData.posMin.x - anchorData.parentPosMin.x) < Mathf.Abs(anchorData.posMin.x - anchorData.parentPos.x)) { anchorMin.x = 0f; }
					if(Mathf.Abs(anchorData.posMin.x - anchorData.parentPosMax.x) < Mathf.Abs(anchorData.posMin.x - anchorData.parentPosMin.x)) { anchorMin.x = 1f; }
					
					// Setup the Min Y
					if(Mathf.Abs(anchorData.posMin.y - anchorData.parentPosMin.y) < Mathf.Abs(anchorData.posMin.y - anchorData.parentPos.y)) { anchorMin.y = 0f; }
					if(Mathf.Abs(anchorData.posMin.y - anchorData.parentPosMax.y) < Mathf.Abs(anchorData.posMin.y - anchorData.parentPosMin.y)) { anchorMin.y = 1f; }


					// Setup the Max X
					if(Mathf.Abs(anchorData.posMax.x - anchorData.parentPosMin.x) < Mathf.Abs(anchorData.posMax.x - anchorData.parentPos.x)) { anchorMax.x = 0f; }
					if(Mathf.Abs(anchorData.posMax.x - anchorData.parentPosMax.x) < Mathf.Abs(anchorData.posMax.x - anchorData.parentPosMin.x)) { anchorMax.x = 1f; }
					
					// Setup the Max Y
					if(Mathf.Abs(anchorData.posMax.y - anchorData.parentPosMin.y) < Mathf.Abs(anchorData.posMax.y - anchorData.parentPos.y)) { anchorMax.y = 0f; }
					if(Mathf.Abs(anchorData.posMax.y - anchorData.parentPosMax.y) < Mathf.Abs(anchorData.posMax.y - anchorData.parentPosMin.y)) { anchorMax.y = 1f; }
				}
				else if(anchorMode == AnchorMode.ClosestSide)
				{
					// Setup the Min X
					if(Mathf.Abs(anchorData.pos.x - anchorData.parentPosMin.x) < Mathf.Abs(anchorData.pos.x - anchorData.parentPos.x)) { anchorMin.x = 0f; }
					if(Mathf.Abs(anchorData.pos.x - anchorData.parentPosMax.x) < Mathf.Abs(anchorData.pos.x - anchorData.parentPosMin.x)) { anchorMin.x = 1f; }

					// Setup the Min Y
					if(Mathf.Abs(anchorData.pos.y - anchorData.parentPosMin.y) < Mathf.Abs(anchorData.pos.y - anchorData.parentPos.y)) { anchorMin.y = 0f; }
					if(Mathf.Abs(anchorData.pos.y - anchorData.parentPosMax.y) < Mathf.Abs(anchorData.pos.y - anchorData.parentPosMin.y)) { anchorMin.y = 1f; }
			...
			return new Vector2[] { anchorMin, anchorMax, new Vector2(anchorData.graphic.leftAnchor.absolute, anchorData.graphic.rightAnchor.absolute), new Vector2( anchorData.graphic.bottomAnchor.absolute, anchorData.graphic.topAnchor.absolute) };
		}
		...
		static Vector2[] DoSetAnchorsNGUI(Transform trans, Transform transParent, Vector2 anchorMin, Vector2 anchorMax)
		{
			...
			return new Vector2[] { anchorMin, anchorMax, anchoredPosAbsMin, anchoredPosAbsMax };
		}
