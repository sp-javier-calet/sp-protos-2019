...
{
	static Object _previousActiveObject = null;
	...
	void OnFocus()
	{
		...
		sceneAssetsScreen.RefreshData(_buildInfo);
		prefabAssetsScreen.RefreshData(_buildInfo);
	...
	void OnEnable()
	{
		...
		if (BuildReportTool.Util.BuildInfoHasContents(_buildInfo))
		{
			...
			else
			{
				...
				if (_buildInfo.HasSceneAssets)
				{
					_buildInfo.SceneAssets.AssignPerCategoryList( BuildReportTool.ReportGenerator.SegregateAssetSizesPerCategory(_buildInfo.SceneAssets.All, _buildInfo.FileFilters) );
				}
				if (_buildInfo.HasPrefabAssets)
				{
					_buildInfo.PrefabAssets.AssignPerCategoryList( BuildReportTool.ReportGenerator.SegregateAssetSizesPerCategory(_buildInfo.PrefabAssets.All, _buildInfo.FileFilters) );
				}
			}
		}
		...
		sceneAssetsScreen.SetListToDisplay(BuildReportTool.Window.Screen.AssetList.ListToDisplay.SceneAssets);
		prefabAssetsScreen.SetListToDisplay(BuildReportTool.Window.Screen.AssetList.ListToDisplay.PrefabAssets);
	...
	void Update()
	{
		...
		if( Selection.activeObject && Selection.activeObject != _previousActiveObject )
		{
			_previousActiveObject = Selection.activeObject;

			if(prefabAssetsScreen.GetPrefabAssets(_buildInfo))
			{
				_selectedCategoryIdx = PREFAB_ASSETS_IDX;
			}
			else if(sceneAssetsScreen.GetSceneAssets(_buildInfo))
			{
				_selectedCategoryIdx = SCENE_ASSETS_IDX;
			}
		}
	}
	...
	BuildReportTool.Window.Screen.AssetList sceneAssetsScreen = new BuildReportTool.Window.Screen.AssetList();
	BuildReportTool.Window.Screen.AssetList prefabAssetsScreen = new BuildReportTool.Window.Screen.AssetList();
	...
	bool IsInSceneAssetsCategory
	{
		get
		{
			return _selectedCategoryIdx == SCENE_ASSETS_IDX;
		}
	}

	bool IsInPrefabAssetsCategory
	{
		get
		{
			return _selectedCategoryIdx == PREFAB_ASSETS_IDX;
		}
	}
	...
	const int SCENE_ASSETS_IDX = 7;
	const int PREFAB_ASSETS_IDX = 8;
	...
	void OnGUI()
	{
		...
		if (GUILayout.Toggle(IsInSceneAssetsCategory, "Scene Assets", BuildReportTool.Window.Settings.TAB_RIGHT_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = SCENE_ASSETS_IDX;
		}
		if (GUILayout.Toggle(IsInPrefabAssetsCategory, "Prefab Assets", BuildReportTool.Window.Settings.TAB_RIGHT_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = PREFAB_ASSETS_IDX;
		}
		...
		if (GUILayout.Toggle(IsInOptionsCategory, _toolbarLabelOptions, Settings.TAB_LEFT_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = OPTIONS_IDX;
		}
		if (GUILayout.Toggle(IsInHelpCategory, _toolbarLabelHelp, Settings.TAB_RIGHT_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = HELP_IDX;
		}*/
		...
		GUILayout.BeginHorizontal();
			...
			GUILayout.BeginVertical();
					...
					else if (IsInSceneAssetsCategory)
					{
						sceneAssetsScreen.DrawGUI(position, _buildInfo);
					}
					else if (IsInPrefabAssetsCategory)
					{
						prefabAssetsScreen.DrawGUI(position, _buildInfo);
					}
