...
public class TouchStick : TouchableControl
{
	...
	public  Vector2		layoutPosPx;		/// Static position calculated by layout 
	private float		layoutRadPx;
	
	public  Vector2		posPx = new Vector2(0, 0);	/// Current static/dynamic position
	private float		radPx = 0;
	
	#region CA vars
	public bool 		DLFollowFinger = true;
	public bool 		DLFollowDelay = false;
	public bool 		DLAnchored = false;
	public bool 		DLDisappear = true;
	public bool 		DLFadeOutGradual   = true;
	float 				DLFollowRadius = 1f;
	float 				fadeAfterSeconds = 1f;
	float 				fadeDuration     = 5f;
	float 				fadeJoystickCounter;
	float 				lastTimeCheckFade;
	public bool			DLScreenLimit = false;
	public float		DLScreenLimitX = 0.5f;
	public float		DLScreenLimitY = 0.2f;
	Vector2 			initpos;
	#endregion
...
*/
	...
	{ 
		...
		{
			this.posPx = new Vector2(Screen.width * 0.1f, Screen.height * 0.1f);
		}
	}
	...
	override public void Init(TouchController joy)
	{
		...
		//initpos =  new Vector2(GetScreenRad() + 15f, Screen.height - GetScreenRad() - 15f);
	...
	private void SetPollPos(Vector2 pos, bool screenPos)
	{
		if (screenPos)		
		{
			if(DLFollowFinger)
			{
				if(Vector2.Distance(pos, this.posPx) > this.DLFollowRadius)
				{
					if(DLFollowDelay)
					{
						this.posPx = Vector2.Lerp(this.posPx, pos + (this.posPx - pos).normalized * this.DLFollowRadius, 0.05f);
					}
					else
					{
						this.posPx = pos + (this.posPx - pos).normalized * this.DLFollowRadius;
					}
				}
			}
	...
	override public void OnUpdate(bool firstUpdate)
	{
		...
		if(!this.pressedCur && this.DLAnchored)
		{
			this.posPx =  new Vector2(GetScreenRad() + 15f, Screen.height - GetScreenRad() - 15f);;
		}
		...
		int touchId, Vector2 touchPos)
	{
		...
		fadeJoystickCounter = 0f;
		lastTimeCheckFade = Time.time;
		...
		int touchId, Vector2 touchPos)
	{
		...
		if(DLScreenLimit)
		{
			if(touchPos.x > Screen.width * DLScreenLimitX)
			{
				OnTouchEnd(touchId);
			}
			if(touchPos.y < Screen.height * DLScreenLimitY)
			{
				OnTouchEnd(touchId);
			}
		}
	...
	override public void DrawGUI()
	{
		...
		if(DLFadeOutGradual)
		{
			fadeJoystickCounter += Time.time - lastTimeCheckFade;
			lastTimeCheckFade = Time.time;
			alphaMultiplier = Mathf.InverseLerp(fadeAfterSeconds + fadeDuration, fadeAfterSeconds, fadeJoystickCounter);
			if(alphaMultiplier < 0.2f) alphaMultiplier = 0.2f;
		}
		
		
		//		if (this.disableGui || ((this.joy.GetAlpha() * this.animAlpha.cur * alphaMultiplier) < 0.001f))
		//			return;
		...
		{		
			if(DLDisappear)
			{
				GUI.color = TouchController.ScaleAlpha(baseColor, this.joy.GetAlpha() * this.animAlpha.cur * alphaMultiplier); //.alphaCur);
			}
			else if(DLFadeOutGradual)
			{
				GUI.color = TouchController.ScaleAlpha(hatColor, alphaMultiplier); //.alphaCur);
			}
		...
		{	
			if(DLDisappear)
			{
				GUI.color = TouchController.ScaleAlpha(hatColor, this.joy.GetAlpha() * this.animAlpha.cur * alphaMultiplier); //.alphaCur);
			}
			else if(DLFadeOutGradual)
			{
				GUI.color = TouchController.ScaleAlpha(hatColor, alphaMultiplier); //.alphaCur);
