...
public class TouchController : MonoBehaviour
{
    ...
    public bool disableWhenNoTouchScreen = false;
    ...
    ///< finger area in centimeters
    private float fingerBufferRadPx = 10;
    // recompute this on res-change
	
    public float stickMagnetAngleMargin	= 10.0f;
    ...
    public float curTime = 0, ///< Internal time
        ...
        invDeltaTime = 60;
    ///< Delta time inverse (for safe division)
    private float lastRealTime;
	
    private bool initialized;
    // flag used to check proper initialization order.
    ...
    ///< Default press animation duration (in seconds)
    public float	releaseAnimDuration = 0.3f;
    ///< Default release animation duration (in seconds)
    public float	disableAnimDuration = 0.3f;
    ///< Default disable animation duration (in seconds)
    public float	enableAnimDuration = 0.3f;
    ///< Default enable animation duration (in seconds)
    public float	cancelAnimDuration = 0.3f;
    ///< Default cancel animation duration (used by dynamic-sticks after forced untouch)
    public float	showAnimDuration = 0.3f;
    ///< Default showing animation duration (in seconds).
    public float	hideAnimDuration = 0.3f;
    ///< Default hiding animation duration (in seconds).
	
	
    public float	releasedZoneScale = 1.0f;
    ///< Global Zone's scale when released.
    public float	pressedZoneScale = 1.1f;
    ///< Global Zone's scale when pressed.
    public float	disabledZoneScale = 1.0f;
    ///< Global Zone's scale when disabled.
	
    public float	releasedStickHatScale	= 0.75f;
    ///< Global Stick Hat's scale when released.
    public float	pressedStickHatScale	= 0.9f;
    ///< Global Stick Hat's scale when pressed.
    public float	disabledStickHatScale	= 0.75f;
    ///< Global Stick Hat's scale when disabled.
    public float	releasedStickBaseScale	= 1.0f;
    ///< Global Stick Base's scale when released.
    public float	pressedStickBaseScale	= 0.9f;
    ///< Global Stick Base's scale when pressed.
    public float	disabledStickBaseScale	= 1.0f;
    ///< Global Stick Base's scale when disabled.
    ...
    private const float
        NON_MOBILE_DIAGONAL_INCHES	= 7;
    // Used for web and standalone demos to calculate virtual DPI
	
	
    private const float 
        DEFAULT_MONITOR_DPI	= 96;
    // Used in case when system's DPI is unknown.
    ...
    // Second finger copies the movement of the mouse.
    public 	KeyCode debugSecondTouchPinchModeKey	= KeyCode.LeftControl;
    // Second finger mirrors horizontal movement of the mouse, allowing horizontal pinch.
    public 	KeyCode debugSecondTouchTwistModeKey	= KeyCode.LeftAlt;
    // Second finger mirrors horizontal and vertical movement of the mouse allowing twist.
    ...
    public bool screenEmuOn = false;
    // Enable screen emulation
    public bool screenEmuPortrait = false;
    // Display in portrait mode (swap horizontal and vertical resolutions)
    public bool screenEmuShrink = true;
    // Allow scaling down when the screen's too small.
    public Vector2 screenEmuPan = new Vector2(0.5f, 0.5f);
    // normalized screen coordinates
    public int screenEmuHwDpi = 250;
    // Emulated device's DPI.
    ...
    public float monitorDiagonal = 15;
    /// monitor's diagonal in inches
    public Color screenEmuBorderColor	= new Color(0, 0, 0, 0.75f);
    // Emulated screen's border color when displayed correctly
    public Color screenEmuBorderBadColor	= new Color(0.5f, 0, 0, 0.75f);
    // Emulated screen's border color when shrunk.
    ...
    ///< Real-world unit used to display coordinates in the editor
    ...
    // script version for future updates...
    ...
    {
        RELEASED,
        ///< Preview controls in released state.
        PRESSED,
        ///< Preview controls in pressed state.
        DISABLED
        ///< Preview controls in disabled state.
    }

	
	
    public const int LayoutBoxCount = 17;
    ...
    public float touchTapMaxDistPx = 10;
    ...
    public enum ControlShape
    {
        ...
        ///< Rectangular box	
        SCREEN_REGION
        ///< Normalized screen region. Doesn't affect layout of other shapes!
    }
    ...
    {
        PIXEL_PERFECT,
        ///< Pixel-perfect display using monitor's DPI.
        PHYSICAL,
        ///< Emulated screen's physical dimensions will match real hardware's - for example, emulated Nexus 7 will take up about a quarter of the screen on a 14-inch monitor.    
        EXPAND
        ///< Emulated screen will expand to window. Good for taking screenshots. Warning - emulated screen may have higher resolution than the actual device.
    }
...
*/
    ...
    private void ResetLayoutBoxes()
    {
        ...
        (this.layoutBoxes[0] == null))
        {
            ...
            for(int i = 0; i < this.layoutBoxes.Length; ++i)
            {
                ...
                {
                case 0:
                    this.layoutBoxes[i] = new LayoutBox(
                ...
                case 5:
                    this.layoutBoxes[i] = new LayoutBox(
                        "Bottom-Right-Qrtr",	0.0f, 0.0f, 1.0f, 1.0f, LayoutAnchor.BOTTOM_RIGHT);
                    break;
					
                case 6:
                    this.layoutBoxes[i] = new LayoutBox(
                        "Bottom-Left-Qrtr", 0.0f, 0.0f, 1.0f, 1.0f, LayoutAnchor.BOTTOM_LEFT);
                    break;
                ...
                case 9:
                    this.layoutBoxes[i] = new LayoutBox(
                        "Full-Screen-Anchor-Bottom-Right", 0.0f, 0.0f, 1.0f, 1.0f, LayoutAnchor.BOTTOM_RIGHT);
                    break;
    ...
    private void PollControllerInternal()
    {
        ...
        var itr = this.touchables.GetEnumerator();
        while(itr.MoveNext())
        {
            itr.Current.OnPrePoll();
        }		
        ...
        {
            itr = this.touchables.GetEnumerator();
            while(itr.MoveNext())
            {
                itr.Current.ReleaseTouches();
            }
    ...
    private void UpdateControllerInternal()
    {
        ...
        if(this.touchables != null)
        {
            ...
            while(itr.MoveNext())
            {
                itr.Current.OnUpdate(this.firstPostPollUpdate);
            }
            ...
            while(itr.MoveNext())
            {
                itr.Current.OnPostUpdate(this.firstPostPollUpdate);
            }
        }
    ...
    public enum EventResult
    {
        ...
        ///< Exclusively handled.
        SHARED
        ///< Touch was handled, but touch sharing is allowed.
    }
    ...
    public struct HitTestResult
    {
        ...
        ///< when comparing two controls of the same priority, distance will be scaled by this value 
        ...
        bool			uniformMargins = false)
    {
        ...
        if(topLeftOfs.x > 0)
            marginX = Mathf.Min(topLeftOfs.x, maxMarginX);
        if(topLeftOfs.y > 0)
            marginY = Mathf.Min(topLeftOfs.y, maxMarginY);
    ...
    public class LayoutBox
    {
        ...
        private Vector2 contentOfs, // raw content rect
            contentSize,
            contentPosScale;
        // raw content pos (in cm) to screen (px)
        private float	contentSizeScale;
        // raw content size (in cm) to uniform screen (px)
