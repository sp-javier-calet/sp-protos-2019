...
namespace SocialPoint.GUIAnimation
{
    public sealed class Snapper
	{
		...
		static void Intersect(List<ResultData> result, List<ResultData> a, List<ResultData> b)
		{
			...
			{
				int foundIdx = b.FindIndex( (ResultData test)=>{ return (test.Rect.position-a[i].Rect.position).sqrMagnitude < 1e-3f; } );
				if( foundIdx >= 0 )
				{
		...
		{
			data.Sort( (ResultData a, ResultData b)=>{ return a.Dist < b.Dist ? -1 : a.Dist > b.Dist ? 1 : 0; } );
		}
	}
}
