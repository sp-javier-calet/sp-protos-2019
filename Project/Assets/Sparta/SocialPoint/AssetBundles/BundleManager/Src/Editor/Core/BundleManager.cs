...
public class BundleManager
{
     ...
     * AKA: If dependence is bundleName or bundlename is a child of dependence
     */
     ...
     */
    public static Action RefreshAll = delegate {
        getInstance().Init();
    };
    ...
    */
    public static void FixAllBundleReferences()
    {
        ...
        foreach(BundleData bdata in allBundles)
        {

            bdata.includs.RemoveAll(x => x.Equals(String.Empty));

            for(int i = 0; i < bdata.includeGUIDs.Count; ++i)
            {
                var includGUID = bdata.includeGUIDs[i];
                if(missingReferences.Contains(includGUID))
                {
                    removeIndexes.Add(i);
                }
                else if(AssetDatabase.GUIDToAssetPath(includGUID).Equals(String.Empty))
                {
                    missingReferences.Add(includGUID);
                    removeIndexes.Add(i);
                }
            }
            removeIndexes.Reverse();
            foreach(int idx in removeIndexes)
                bdata.includeGUIDs.RemoveAt(idx);
            removeIndexes.Clear();

            bdata.dependAssets.RemoveAll(x => x.Equals(String.Empty));

            for(int i = 0; i < bdata.dependGUIDs.Count; ++i)
            {
                var dependGUID = bdata.dependGUIDs[i];
                if(missingReferences.Contains(dependGUID))
                {
                    removeIndexes.Add(i);
                }
                else if(AssetDatabase.GUIDToAssetPath(dependGUID).Equals(String.Empty))
                {
                    missingReferences.Add(dependGUID);
                    removeIndexes.Add(i);
                }
            }
            removeIndexes.Reverse();
            foreach(int idx in removeIndexes)
                bdata.dependGUIDs.RemoveAt(idx);
            removeIndexes.Clear();
        }
