...
public class AssetBundleAnalyzer : EditorWindow {
    ...
    void OnGUI()
    {
        ...
        if (Selection.activeObject != null) {
            string loadedPath = AssetDatabase.GetAssetPath(Selection.activeObject);

            if (loadedPath != ""){
                assetPath = Path.GetFullPath(loadedPath);

                if (assetPath.ToLower().Contains(".assetbundle")) {
        ...
        if (currentObject != null && assetPath != "") {
