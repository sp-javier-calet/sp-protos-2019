...
namespace SocialPoint.Tool.Shared.TLGUI
{
	...
	public class TLWTreeFilteredSelector<T> : TLWidget where T : TLTreeSelectorItem<T>
	{	
        ...
        protected TLEvent<string> _onSearchTextChanged;
        ...
        protected T[]       _itemsArr = new T[0]; // items in array structure, this should be faster for drawing
        protected T[]       _filteredItems = new T[0];
        protected bool      includeSelected;
		...
		public TLWTreeFilteredSelector( TLView view, string name ): base ( view, name, TLLayoutOptions.noexpand )
		{
			Init ();
		}

		public TLWTreeFilteredSelector( TLView view, string name, GUILayoutOption[] options ): base ( view, name, options )
		{
			Init ();
		}
        ...
        protected virtual void Init(TLStyle style = null)
        {
            ...
            _onSearchTextChanged = new TLEvent<string>("OnSearchTextChanges");
            ...
            _searchFieldLayoutOpts = new GUILayoutOption[] { GUILayout.MinWidth (150), GUILayout.MinHeight (20),
                GUILayout.ExpandWidth (false), GUILayout.ExpandHeight (false), GUILayout.MaxWidth(150) };

            _onSearchTextChanged.Connect(FilterContent);
            _onItemsChanged.Connect(OnItemsChanged);
        }


        void FilterContent(string rawFilter)
        {
            ...
            if (string.IsNullOrEmpty(rawFilter))
                ...
                filter = "^.*" + Regex.Escape(rawFilter.ToUpper()).Replace(@"\*", ".*").Replace(@"\ ", ".*").Replace(@"\?", ".") + ".*$";

            var allItems = GetListItems(excludeChilds: false);

            for (int i = 0; i < allItems.Count; ++i)
            {
                if (allItems[i].GetParent() == null)
                {
                    bool parentIn = false;
                    var content = allItems[i].Content;
                    if (Regex.IsMatch(content.ToUpper(), filter) || (includeSelected && allItems[i].IsSelected()))
                    {
                        _FilteredItems.Add(allItems[i]);
                        parentIn = true;
                    }

                    if (!parentIn)
                    {
                        foreach (T item in allItems[i].GetChilds())
                        {
                            var childContent = item.Content;
                            if (Regex.IsMatch(childContent.ToUpper(), filter) || (includeSelected && item.IsSelected()))
                            {
            ...
            _filteredItems = _FilteredItems.ToArray();                       
		...
		public void SetListItems (List<T> items, bool sorted=false)
		{
			...
			if (_items != null) {
                if (sorted)
				    _items.Sort (TLWTreeFilteredSelector<T>.CompareItemsByName);
        ...
        public virtual void DrawSearch()
        {
            ...
            if (isSearchFocused)
            {
                ...
                if (!_textCleared)
                {
                    ...
                    EditorGUILayout.TextField("", TLTreeSelectorStyles.InnerSearchFieldStyle.GetStyle(), _searchFieldLayoutOpts);
                    EditorGUI.FocusTextInControl(_searchControlName);
                    _textCleared = true;
                }
                else
                {
                    string newText = EditorGUILayout.TextField(SearchText, TLTreeSelectorStyles.InnerSearchFieldStyle.GetStyle(), _searchFieldLayoutOpts);
            ...
            else
            {
                ...
                {
                    EditorGUILayout.TextField ("Filter...", TLTreeSelectorStyles.InnerSearchFieldInactiveStyle.GetStyle (), _searchFieldLayoutOpts);
                    _textCleared = false;
                }
                else
                {
                    EditorGUILayout.TextField (SearchText, TLTreeSelectorStyles.InnerSearchFieldStyle.GetStyle (), _searchFieldLayoutOpts);
                }
            }
        }
        ...
        void BasicDraw()
        {
            if (_itemsArr != null) {
                ...
                for (int i = 0; i < _itemsArr.Length; ++i)
                    _filteredItems[i].Draw(ref baseSelected);
        ...
        void OptimizedDraw()
        {
            if (_itemsArr != null) {
                ...
                for (int i = 0; i < _filteredItems.Length; ++i){
                    ...
                    accumHeight += _filteredItems[i].TotalHeight;

                    if(accumHeight < displayHeightStart)
                        preItemSpace = accumHeight;
                    else if(prevAccumheight > displayHeightEnd)
                        postItemSpace += _filteredItems[i].TotalHeight;
                    else
                    {
                        displyedItems.Add(_filteredItems[i]);
                    }
                }
        ...
        public void SearchWord(string word)
        {
            ...
            if (word != "" && _items != null)
            {
                foreach (T item in _items)
                {
                    if (item.Depth == 0 && item.Content.StartsWith(word, true, CultureInfo.GetCultureInfo(0x000A)))
                    {
                        if (Selected != item)
                        {
                            Selected = item;
        ...
        public void SetFilteredContent(List<string> filteredItems)
        {
            List<T> aux = new List<T>();

            for(int i = 0; i < _itemsArr.Length; i++)
            {
                if (filteredItems.Contains(_itemsArr[i].Content))
                {
                    aux.Add(_itemsArr[i]);
                }
            }
            _filteredItems = aux.ToArray();
