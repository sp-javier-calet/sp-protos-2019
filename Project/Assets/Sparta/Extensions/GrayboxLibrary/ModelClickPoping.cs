...
public class ModelClickPoping : MonoBehaviour {
    ...
    void Awake ()
    {
        ...
        _originalOverbright = _mat.GetFloat("_Overbright");
	...
	void Update ()
    {
        ...
        {
            _mat.SetFloat("_Overbright", _currentOverbrigth);
            _currentOverbrigth = Mathf.Max(_currentOverbrigth - _ovebrightSpeed, _originalOverbright);
        }

        if (Vector3.Distance(_currentSize, _targetSize) != 0 && _targetSize == _bigSize)
        {
            transform.localScale = _currentSize;

            float frame = (Time.time - _startTime) * SizeSpeed;
            float fraction = frame / _sizeDiference;
            _currentSize = Vector3.Lerp(_originalSize, _bigSize, fraction);
        }
        else if(Vector3.Distance(_currentSize, _targetSize) != 0 && _targetSize == _originalSize)
        {
            transform.localScale = _currentSize;

            float frame = (Time.time - _startTime) * SizeSpeed;
            float fraction = frame / _sizeDiference;
            _currentSize = Vector3.Lerp(_bigSize, _originalSize, fraction);
        }
