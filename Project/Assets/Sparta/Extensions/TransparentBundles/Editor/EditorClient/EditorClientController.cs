    ...
    public class EditorClientController
    {
        ...
        Dictionary<string, Bundle> ReadBundleListFromJSON(byte[] jsonBytes)
        {
            ...
            for(int i = 0; i < jsonList.Count; i++)
            {
                ...
                //TEMPORARY
                if(NewBundles.ContainsKey(bundleName) && !bundleDictionary[GetFixedAssetName(asset.Name)].IsAutogenerated)
                {
            ...
            while(newBundleEnum.MoveNext())
            {
                ...
                string newBundleAssetName = GetFixedAssetName(newBundle.Asset.Name);
        ...
        {
            if((!_bundleDictionary.ContainsKey(GetFixedAssetName(asset.Name)) || _bundleDictionary[GetFixedAssetName(asset.Name)].IsAutogenerated) && !NewBundles.ContainsKey(asset.FullName.ToLower().Replace(".", "_").Replace(" ", "_")))
            {
        ...
        public bool CreateOrUpdateBundles(List<Asset> assets)
        {
            ...
            for(int i = 0; i < assets.Count; i++)
            {
                ...
                {
                    string fixedAssetName = GetFixedAssetName(asset.Name);
