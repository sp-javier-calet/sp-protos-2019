...
namespace SocialPoint.TransparentBundles
{
    public static class GitChecker
    {
        ...
        public static List<string> CheckInBranchUpdated(string branchName)
        {
            ...
            {
                infractions.Add(string.Format("The current branch is {0}. The configured branch for Transparent Bundles is {1}", currentBranch, branchName));
            }

            var queryLocal = repo.CreateQuery("log").WithArg("-1").WithOption("oneline").WithArg(currentBranch);
            var queryRemote = repo.CreateQuery("log").WithArg("-1").WithOption("oneline").WithArg("origin/" + currentBranch);
