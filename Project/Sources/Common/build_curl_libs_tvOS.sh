...
function setup_lib {
    ...
    FLAGS="-arch "${ARCH}" -pipe -isysroot "${SDK_ROOT}" -m${BITS} -mappletvos-version-min=9.0 -DAPPLETV=APPLETV -DAPPLETVOS=APPLETVOS -fembed-bitcode -DENABLE_BITCODE=YES"
    echo "FLAGS: ${FLAGS}"
    export CFLAGS="${FLAGS}"
...
function build_openssl {
    ...
    then
        ...
        setup_lib "${ARCH}"
        export CFLAGS="${CFLAGS} -DHAVE_FORK=0"
...
function build_curl {
    ...
    then
      echo "building curl for arch ${ARCH}..."
        ...
        setup_lib "${ARCH}"
        export CFLAGS="${CFLAGS} -DCURL_DISABLE_NTLM=CURL_DISABLE_NTLM"
        ...
        ./configure --disable-shared --enable-static --enable-threaded-resolver \
            ...
            --without-libidn --host="${HOST}-apple-darwin" > /dev/null
        make_lib -j > /dev/null
...
echo "building curl for tvOS in ${BUILD_DIR}..."
mkdir -p "${BUILD_DIR}"
