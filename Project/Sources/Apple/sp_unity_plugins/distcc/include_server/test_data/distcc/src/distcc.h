...
/* hosts.c */
int dcc_parse_hosts_env(struct dcc_hostdef** ret_list, int* ret_nhosts);
int dcc_parse_hosts(const char* where, const char* source_name, struct dcc_hostdef** ret_list, int* ret_nhosts);
...
/* clirpc.c */
int dcc_x_req_header(int fd, enum dcc_protover protover);
...
/* remote.c */
int dcc_send_job_corked(int net_fd, char** argv, pid_t cpp_pid, int* status, const char*, const char* cpp_fname, struct dcc_hostdef*);

int dcc_retrieve_results(int net_fd, int* status, const char* output_fname, const char* deps_fname, const char* server_stderr_fname,
                         struct dcc_hostdef*);
...
/* compress.c */
int dcc_r_bulk_lzo1x(int outf_fd, int in_fd, unsigned in_len);


int dcc_compress_file_lzo1x(int in_fd, size_t in_len, char** out_buf, size_t* out_len);

int dcc_compress_lzo1x_alloc(const char* in_buf, size_t in_len, char** out_buf_ret, size_t* out_len_ret);
...
int dcc_scan_args(char* argv[], /*@out@*/ /*@relnull@*/ char** orig_o, char** orig_i, char*** ret_newargv);
...
int dcc_output_from_source(const char* sfile, const char* out_extn, char** ofile);
...
/* loadfile.c */
int dcc_load_file_string(const char* filename, char** retbuf);
...
/* pump.c */
int dcc_r_bulk(int ofd, int ifd, unsigned f_size, enum dcc_compress compression);
