...
SPUnityCrashReporterBridge* s_SPUnityCrashReporterBridge = nullptr;
...
{
    if(s_SPUnityCrashReporterBridge == nullptr)
    {
        s_SPUnityCrashReporterBridge = new SPUnityCrashReporterBridge();
    }
    s_SPUnityCrashReporterBridge->setConfig(std::string(path), std::string(version), std::string(fileSeparator), std::string(crashExtension), callback);
}
...
EXPORT_API void SPUnityCrashReporter_Enable()
{
    if(s_SPUnityCrashReporterBridge != nullptr)
    {
        s_SPUnityCrashReporterBridge->enable();
    }
}
...
EXPORT_API void SPUnityCrashReporter_Disable()
{
    if(s_SPUnityCrashReporterBridge != nullptr)
    {
        s_SPUnityCrashReporterBridge->disable();
    }
}
